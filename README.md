...
├── minikv\_client.py        # Python 客户端：和 C++ 服务端通信（TCP 文本协议）

├── mysql\_store.py          # MySQL 存储层：备份数据的读写（pymysql）
...
│   ├── step1\_mysql.py      #   Python 连 MySQL 的最小代码

│   └── step2\_cpp.py        #   Python 连 C++ 服务端的最小代码

└── tests/

&#x20;   ├── test\_client.py      # 只测 Python <-> C++ 通信（不需要 MySQL）

&#x20;   └── test\_e2e.py         # 端到端：备份 -> 清空 -> 恢复 -> 校验（需要 MySQL）
```
...
```
conda create -n minikv\_backup python=3.12 -y

conda activate minikv\_backup
...
1. **建议建一个专用账号**（别让代码里写死 root 密码，安全习惯）：
...
```
\# ❌ 错误：如果 key 是 "x'; DROP TABLE kv\_snapshot; --" 就完蛋了

cursor.execute(f"SELECT \* FROM kv\_snapshot WHERE k = '{key}'")

\# ✅ 正确：用 %s 占位，数据单独传参，pymysql 帮你转义

cursor.execute("SELECT \* FROM kv\_snapshot WHERE k = %s", (key,))
```
...
```
data = \[(1, "a", "v1"), (1, "b", "v2")]          # \[(batch\_id, k, v), ...]

cursor.executemany(

&#x20;   "INSERT INTO kv\_snapshot (batch\_id, k, v) VALUES (%s, %s, %s)",
...
sock = socket.create\_connection(("127.0.0.1", 9000), timeout=5)
...
```
欢迎语： WELCOME mini\_kv\_server. Type HELP for commands.
...
```
备份：MiniKVClient.keys() -> GET 逐个取值 -> KVStore.create\_snapshot()

恢复：KVStore.load\_snapshot() -> MiniKVClient.set() 逐条写回
```
...
```
../mini\_kv\_server/build/Release/mini\_kv\_server.exe --port 9000 --workers 4 --queue 64 --backlog 64
```
...
```
python -c "from minikv\_client import MiniKVClient; c=MiniKVClient(); c.connect(); \[c.set(f'user:{i}', f'value-{i}') for i in range(20)]; print('size =', c.size()); c.close()"
```
...
```
mysql -u root -p -e "USE minikv; SELECT \* FROM kv\_snapshot LIMIT 5;"
```
...
```
python -c "from minikv\_client import MiniKVClient; c=MiniKVClient(); c.connect(); \[c.delete(k) for k in c.keys()]; print('size =', c.size()); c.close()"
```
...
```
python -c "from minikv\_client import MiniKVClient; c=MiniKVClient(); c.connect(); print('size =', c.size(), '| user:0 =', c.get('user:0')); c.close()"
```
...
```
python tests/test\_e2e.py --db-password 你的密码
```
...
1. **快照不是原子的**：备份过程中服务端可能还在写入，KEYS 之后新 SET 的 key

   这次备份里没有。可以怎么改进？（服务端加一个 "只读 dump" 命令 / 或先做一致性标记。）

2. **幂等恢复**：SET 是覆盖写，重复执行恢复结果不变，这是运维友好的重要性质。

3. **批量写 vs 单条写**：executemany 一次网络往返写 N 条，数据量大时差距明显。

4. **半包粘包**：C++ 端和 Python 端用同一套 "缓冲 + 找分隔符" 的思路，

   说明你理解 TCP 是字节流而不是消息流。

5. **事务**：写库要么全成要么全不成，commit/rollback 保证不留半截数据。
